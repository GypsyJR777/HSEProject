...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        '''
        global mdf, tree, mxls1, mxls2, mxls3
        ...
        try:
            ...
            xls3 = pd.read_pickle('../Data/Pickle3.pkl')
            xls1 = pd.read_pickle('../Data/Pickle1.pkl')
            ...
            xls1 = pd.DataFrame(columns=['Manufacturer', 'Country'])
            xls2 = pd.DataFrame(columns=["Product Code", "Storage", "CPU", "RAM", "Amount", "Model"])
            xls3 = pd.DataFrame(columns=["Model", "Diagonal", "OS", "Manufacturer"])

        mdf = pd.merge(pd.merge(xls2, xls3, on=('Model')).drop_duplicates(subset=['Product Code']), xls1, on=('Manufacturer'))
        mxls1=xls1
        mxls2=xls2
        mxls3=xls3
        bd.Table(frame_table, xls1, xls2, xls3)
